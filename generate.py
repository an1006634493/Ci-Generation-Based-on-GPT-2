...
def main(input1, input2, input3, input4):
    ...
    while True:
        ...
        if generated == nsamples:
            ...
            break
    # pdb.set_trace()
    text_list = text.split('词牌名')[-1].split('。')[:-1]
    text = "。\n".join(text_list)
    # pdb.set_trace()
    return text

parser = argparse.ArgumentParser()
parser.add_argument('--device', default='6', type=str, required=False, help='生成设备')
...
parser.add_argument('--topp', default=5, type=float, required=False, help='最高积累概率')
parser.add_argument('--model_config', default='config/model_config_small.json', type=str, required=False,
                    help='模型参数')
parser.add_argument('--tokenizer_path', default='cache/vocab_small.txt', type=str, required=False, help='词表路径')
parser.add_argument('--model_path', default='model/model_epoch60', type=str, required=False, help='模型路径')
parser.add_argument('--prefix', default='[MASK]', type=str, required=False, help='生成文章的开头')
...
input1 = gradio.inputs.Textbox(lines=1, label="词牌名", placeholder="浣溪沙", default="")
input2 = gradio.inputs.Dropdown(["手动输入", "浣溪沙", "水调歌头", "鹧鸪天", "菩萨蛮"], label="常用词牌名")
input3 = gradio.inputs.Textbox(lines=1, label="自定主题", placeholder="柳花", default="")
input4 = gradio.inputs.Dropdown(["手动输入","柳花", "清明", "春水", "绿柳"], label="常用主题")
output = gradio.outputs.Textbox(label="宋词")
gradio.Interface(fn=main,
    inputs=[input1, input2, input3, input4], outputs=output, title="基于歪比巴卜的宋词生成器").launch(share=True)
# main('[MASK]浣溪沙词牌名柳花')

#     ssh -L 7860:127.0.0.1:7860 amax_user7@202.117.63.3 -N -v -v
